...
def test_contact_force_lifts_particle_above_plane():
    ...
    assert sim.positions[0].z > before


def test_higher_drag_reduces_displacement():
    ...
    assert abs(low_drag.positions[front_index].x - low_before) > abs(high_drag.positions[front_index].x - high_before)


def test_spring_network_is_stable_for_short_run():
    sim = SoftCellSimulation(SoftCellParameters(lat_segments=4, lon_segments=8))

    for _ in range(30):
        sim.step(1.0 / 60.0)

    assert len(sim.positions) == 40
    assert all(position.z > -0.05 for position in sim.positions)
